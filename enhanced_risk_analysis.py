...
class EnhancedRiskAnalysis:
    ...
    def create_feature_matrix(self, agriculture_df, temp_df, land_df):
        ...
        # Calculate risk metrics with normalized values
        features['production_volatility'] = features.groupby('Item')['Value'].transform(
            lambda x: x.rolling(3, min_periods=1).std() / x.rolling(3, min_periods=1).mean()
        ).fillna(0)
        
        features['climate_sensitivity'] = (
            features['production_volatility'] * 
            features['temp_volatility'].fillna(0) / 
            features['temp_volatility'].max()  # Normalize temperature volatility
        )
        
        features['land_climate_risk'] = (
            (features['temp_volatility'] / features['temp_volatility'].max()) * 
            (1 - features['land_utilization'])
        )
        ...
        ).clip(0, 100)  # Cap the maximum risk score
